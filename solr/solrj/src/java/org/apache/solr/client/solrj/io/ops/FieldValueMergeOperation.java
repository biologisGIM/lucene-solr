...
public class FieldValueMergeOperation implements ReduceOperation {
  ...
  public FieldValueMergeOperation(StreamExpression expression, StreamFactory factory) throws IOException {
    ...
    StreamExpressionNamedParameter mergeFieldParam = factory.getNamedOperand(expression, "mergeField");
    ...
    String mergeFieldName = ((StreamExpressionValue)mergeFieldParam.getParameter()).getValue();
    int nInt = 0;
    ...
    init(streamComparator, nInt, mergeFieldName);
  }

  public FieldValueMergeOperation(StreamComparator streamComparator, int size, String mergeFieldName) {
    init(streamComparator, size, mergeFieldName);
  }

  private void init(StreamComparator streamComparator, int size, String mergeFieldName) {
    this.size = size;
  ...
  public StreamExpressionParameter toExpression(StreamFactory factory) throws IOException {
    ...
    // mergeField
    expression.addParameter(new StreamExpressionNamedParameter("mergeField", mergeFieldName));
    return expression;
  }
  ...
  public Tuple reduce() {
    ...
    while(priorityQueue.size() > 0) {
      currentMap = priorityQueue.poll().getMap();
      if (firstMap == null) {
        firstMap = currentMap;
      }
      value = currentMap.get(mergeFieldName);
      if (value != null) {
    ...
    if (mergedValues.isEmpty()) {
      map.remove(mergeFieldName);
    }
    else {
      map.put(mergeFieldName, mergedValues);
    }
